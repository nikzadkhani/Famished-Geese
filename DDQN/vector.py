...
def create_norm_state_vector(
        state: kaggle_environments.utils.Struct,
        previous_state: kaggle_environments.utils.Struct) -> np.array:
    def centerize(b):
        dy, dx = np.where(b[0])
    ...
    vector = vector.reshape(17, 7, 11)

    # If state is terminal
    if 1 not in vector[0]:
        return None

    vector = centerize(vector)


    vector = vector.reshape(-1, 17, 7, 11)
