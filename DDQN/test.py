...
from vector import get_action,create_state_vector 
...
def test_single_episode_with_trainer(ddqn: DDQN, render: bool) -> Tuple[bool, int]:
  observations = []
  action_vectors = []
  actions = []
  state = trainer.reset()
...
def test(ddqn: DDQN, num_episodes: int, render=False) -> None:
  ...
  for i in trange(num_episodes):
    was_win, player_goose_length = test_single_episode_with_trainer(ddqn, render)
